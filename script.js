...
function formatTime(seconds) {
  ...
  if (seconds < 3153600000) {
    ...
    return `🗓️ ${years} ${years === 1 ? 'سنة' : 'سنوات'}`;
  }

  // عقود (100 - 1000 سنة)
  if (seconds < 31536000000) {
    const decades = Math.round(seconds / 3153600000);
    return `🛡️ ${decades} عقد`;
  }

  // قرون (1000+ سنة)
  if (seconds < 3153600000000) {
    const centuries = Math.round(seconds / 31536000000);
    return `🛡️ ${centuries} قرن`;
  }

  // أكثر من ألف قرن
  return '🛡️ غير قابلة للاختراق';
}
